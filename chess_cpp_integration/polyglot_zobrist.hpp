/**
 * polyglot_zobrist.hpp - Polyglot Zobrist hash constants
 * 
 * These are the standard Polyglot random numbers used for Zobrist hashing.
 * Compatible with python-chess's chess.polyglot.zobrist_hash()
 */

#ifndef POLYGLOT_ZOBRIST_HPP
#define POLYGLOT_ZOBRIST_HPP
...
// Array layout:
// [0-767]   = pieces: 64 squares * 12 piece types (piece_type-1 + 6*color) * 64 + square
// [768-771] = castling rights (white kingside, white queenside, black kingside, black queenside)
// [772-779] = en passant file (a-h)
// [780]     = side to move (white)

constexpr uint64_t RANDOM_ARRAY[781] = {
    // Piece keys [0-767]: indexed by (piece_type-1 + 6*color)*64 + square
    // Black Pawn (index 0-63)
    ...
    0x65D34954DAF3AB0FULL, 0xB4B81B3FA97511E2ULL, 0xB422061193D6F6A7ULL, 0x071582401C38434DULL,
    ...
    // Castling keys [768-771]
    0x31D71DCE64B2C310ULL,  // White kingside
    0xF165B587DF898190ULL,  // White queenside
    0xA57E6339DD2CF3A0ULL,  // Black kingside
    0x1EF6E6DBB1961EC9ULL,  // Black queenside
    // En passant file keys [772-779]
    0x70CC73D90BC26E24ULL,  // a-file
    0xE21A6B35DF0C3AD7ULL,  // b-file
    0x003A93D8B2806962ULL,  // c-file
    0x1C99DED33CB890A1ULL,  // d-file
    0xCF3145DE0ADD4289ULL,  // e-file
    0xD0E4427A5514FB72ULL,  // f-file
    0x77C621CC9FB3A483ULL,  // g-file
    0x67A34DAC4356550BULL,  // h-file
    // Side to move key [780]
    0xF8D626AAAF278509ULL   // White to move
};

// Index constants
constexpr int CASTLING_BASE = 768;
constexpr int EP_BASE = 772;
constexpr int TURN_KEY = 780;

// Piece type to Polyglot piece index mapping
// Polyglot uses: pawn=0, knight=1, bishop=2, rook=3, queen=4, king=5
// With black pieces at offset 0, white at offset 6
inline int piece_index(int piece_type, bool color) {
    // piece_type: 1=pawn, 2=knight, 3=bishop, 4=rook, 5=queen, 6=king
    // Polyglot order: for each piece type, black then white
    // index = (piece_type - 1) * 2 + color_index
    // where color_index: black=0, white=1
    int pt = piece_type - 1;  // Convert to 0-based (pawn=0, knight=1, etc.)
    int color_idx = color ? 1 : 0;  // white=1, black=0
    return pt * 2 + color_idx;
}

inline uint64_t piece_key(int piece_type, bool color, int square) {
    return RANDOM_ARRAY[piece_index(piece_type, color) * 64 + square];
}

} // namespace polyglot

#endif // POLYGLOT_ZOBRIST_HPP