...
The objectives of this project are to demonstrate 1) chess position evaluation using deep neural network (DNN) 
and 2) a chess engine that can play at a strength equal to an experienced club player, i.e., about 1500 ELO 
rating strength.

## Description

The project used about 100 million chess positions to train a DNN that can evaluate a chess position. The 
chess engine uses the chess position evaluation provided by the DNN to decide the best move by evaluating 
...
PyChess as well other chess GUI that support UCI. The key features of the chess engine are:
...
* Set up the conda environment using the environment.yml file
  * Build DNN model (optional). Note: You may skip this step and use the prebuilt model/model.keras
    ...
    * Convert the pgn.zst files into multiple (about 100) one-hot encoded files by running the command 
     'python prepare_day.py filename.pgn.zst'.  Note: the conversion will take 2 to 3 days to complete.
