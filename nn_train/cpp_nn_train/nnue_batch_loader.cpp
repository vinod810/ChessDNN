/**
 * batch_loader.cpp - High-performance C++ batch loader for NNUE training
 * 
 * Compile with:
 *   g++ -O3 -march=native -shared -fPIC -o libbatch_loader.so batch_loader.cpp -lzstd -lpthread -std=c++17
 * 
...
struct Position {
    int16_t score_cp;
    uint8_t stm;  // 1 = white, 0 = black
    std::vector<uint16_t> white_features;
    std::vector<uint16_t> black_features;
};
...
struct OwnedBatch {
    ...
    // Owned storage
    std::vector<int64_t> white_position_indices;
    std::vector<int64_t> white_feature_indices;
    std::vector<int64_t> black_position_indices;
    std::vector<int64_t> black_feature_indices;
    std::vector<float> stm;
    ...
    void resize(int32_t batch_size, int32_t max_white_features, int32_t max_black_features) {
        white_position_indices.resize(max_white_features);
        white_feature_indices.resize(max_white_features);
        black_position_indices.resize(max_black_features);
        black_feature_indices.resize(max_black_features);
        stm.resize(batch_size);
        scores.resize(batch_size);
        
        // Update pointers
        batch.white_position_indices = white_position_indices.data();
        batch.white_feature_indices = white_feature_indices.data();
        batch.black_position_indices = black_position_indices.data();
        batch.black_feature_indices = black_feature_indices.data();
        batch.stm = stm.data();
...
class ShardReader {
public:
    ShardReader() : dctx_(ZSTD_createDCtx()) {}
    ...
    bool read_shard(const std::string& path, std::vector<Position>& positions) {
        positions.clear();
        ...
        // Parse positions
        return parse_nnue_positions(decompressed_buffer_.data(), result, positions);
    }
    
    const std::string& get_error() const { return error_; }

private:
    bool parse_nnue_positions(const uint8_t* data, size_t size, std::vector<Position>& positions) {
        size_t offset = 0;
        ...
        while (offset < size) {
            ...
            if (first_byte == 0xFF) {
                // Might be diagnostic record - try to parse it
                ...
                if (stm > 1) {
                    ...
                    if (offset >= size) break;
                    uint8_t num_white = data[offset++];

                    // Validate: max 30 non-king pieces
                    if (num_white > 30) {
                        offset = saved_offset;
                    } else {
                        // Skip white features
                        bool valid = true;
                        for (int i = 0; i < num_white && valid; i++) {
                            if (offset + 2 > size) { valid = false; break; }
                            uint16_t feat = data[offset] | (data[offset + 1] << 8);
                            if (feat >= 40960) valid = false;
                            offset += 2;
                        }
                        ...
                        if (!valid) {
                            ...
                            if (offset >= size) break;
                            uint8_t num_black = data[offset++];

                            if (num_black > 30) {
                                offset = saved_offset;
                            } else {
                                // Skip black features
                                for (int i = 0; i < num_black && valid; i++) {
                                    if (offset + 2 > size) { valid = false; break; }
                                    uint16_t feat = data[offset] | (data[offset + 1] << 8);
                                    if (feat >= 40960) valid = false;
                                    offset += 2;
            ...
            // Normal record: [score:int16][stm:uint8][num_white:uint8][white:uint16[]]
            ...
            uint8_t num_white = data[offset++];
            if (num_white > 30) {
                // Invalid, skip this record or abort
                error_ = "Invalid num_white: " + std::to_string(num_white);
                // Try to continue by skipping
                continue;
            }
            pos.white_features.reserve(num_white);

            for (int i = 0; i < num_white; i++) {
                if (offset + 2 > size) {
                    error_ = "Unexpected end of data reading white features";
                    return true;  // Return what we have so far
                }
                uint16_t feature = data[offset] | (data[offset + 1] << 8);
                if (feature >= 40960) {
                    error_ = "Invalid white feature index: " + std::to_string(feature);
                    return true;  // Return what we have so far
                }
                pos.white_features.push_back(feature);
                offset += 2;
            }
            ...
            uint8_t num_black = data[offset++];
            if (num_black > 30) {
                error_ = "Invalid num_black: " + std::to_string(num_black);
                continue;
            }
            pos.black_features.reserve(num_black);

            for (int i = 0; i < num_black; i++) {
                if (offset + 2 > size) {
                    error_ = "Unexpected end of data reading black features";
                    return true;  // Return what we have so far
                }
                uint16_t feature = data[offset] | (data[offset + 1] << 8);
                if (feature >= 40960) {
                    error_ = "Invalid black feature index: " + std::to_string(feature);
                    return true;  // Return what we have so far
                }
                pos.black_features.push_back(feature);
                offset += 2;
            }
...
public:
        ...
        running_(false)
    {
        ...
        current_batch_.resize(batch_size, max_features, max_features);
    }
    ...
    SparseBatch* get_batch() {
        std::unique_ptr<OwnedBatch> batch;
...
private:
    ...
    std::unique_ptr<OwnedBatch> build_batch(const std::vector<Position>& positions) {
        auto batch = std::make_unique<OwnedBatch>();

        int32_t batch_size = positions.size();

        // Count total features
        int32_t total_white = 0;
        int32_t total_black = 0;
        for (const auto& pos : positions) {
            total_white += pos.white_features.size();
            total_black += pos.black_features.size();
        }

        // Resize buffers
        batch->resize(batch_size, total_white, total_black);

        // Fill in data
        int32_t white_offset = 0;
        int32_t black_offset = 0;

        for (int32_t i = 0; i < batch_size; i++) {
            const auto& pos = positions[i];

            batch->stm[i] = pos.stm ? 1.0f : 0.0f;
            batch->scores[i] = static_cast<float>(pos.score_cp);

            // White features (sorted for coalesced tensor)
            std::vector<uint16_t> sorted_white = pos.white_features;
            std::sort(sorted_white.begin(), sorted_white.end());

            for (uint16_t feat : sorted_white) {
                batch->white_position_indices[white_offset] = i;
                batch->white_feature_indices[white_offset] = feat;
                white_offset++;
            }

            // Black features (sorted for coalesced tensor)
            std::vector<uint16_t> sorted_black = pos.black_features;
            std::sort(sorted_black.begin(), sorted_black.end());

            for (uint16_t feat : sorted_black) {
                batch->black_position_indices[black_offset] = i;
                batch->black_feature_indices[black_offset] = feat;
                black_offset++;
            }
        }
        ...
        batch->batch.num_white_features = total_white;
        batch->batch.num_black_features = total_black;
    ...
    // Batch queue
    ThreadSafeQueue<std::unique_ptr<OwnedBatch>> batch_queue_;

    // Current batch (for stable pointer return)
    OwnedBatch current_batch_;
...
BatchLoaderHandle batch_loader_create(
    ...
    try {
        ...
        auto loader = new BatchLoader(
            paths, batch_size, num_workers, queue_size,
            num_features, shuffle != 0, seed
        );

        return static_cast<BatchLoaderHandle>(loader);
    } catch (const std::exception& e) {
        std::cerr << "batch_loader_create failed: " << e.what() << std::endl;
        return nullptr;
    }
}
...
void batch_loader_start(BatchLoaderHandle handle) {
    if (!handle) return;
    auto loader = static_cast<BatchLoader*>(handle);
    loader->start();
}
